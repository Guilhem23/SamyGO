...
#define ECHOA_CHIP_ID	0xAE
		...
		char* kernel_cmdline_ext0="root=/dev/mmcblk0p13 console=ttyS1,115200n8 SELP_ENABLE=20102011 onboot="SAMSUNG_BOOTLOADER_VERSION" \0";
		char* kernel_cmdline_ext1="at 14 console=ttyS1,115200n8 SELP_ENABLE=20102011 onboot="SAMSUNG_BOOTLOADER_VERSION" \0";
		char* kernel_cmdline_sda1="root=/dev/sda1 console=ttyS1,115200n8 SELP_ENABLE=20102011 onboot="SAMSUNG_BOOTLOADER_VERSION" \0";
		char* kernel_cmdline_nfs="root=/dev/nfs console=ttyS1,115200n8 SELP_ENABLE=20102011 onboot="SAMSUNG_BOOTLOADER_VERSION" \0";
		#else
...
unsigned char default_env(void) {
	
	setenv("bootargs" , "root=/dev/mmcblk0p14 console=ttyS1,115200n8 SELP_ENABLE=20102011 onboot=053 quiet");
	setenv("bootdelay", "3000");
	setenv("bootparition", "1");
	setenv("loadaddr", "0x40008000");
	setenv("rootpath", "/mnt/disk1/TV_rootfs");
	setenv("ipaddr", "192.168.0.11");
	setenv("gatewayip", "192.168.0.252");
	setenv("netmask", "255.255.255.0");
	setenv("ethaddr", "C4:73:1E:77:87:FE");
...
/* Code Implementation                                                       */
/*****************************************************************************/
inline void markKernel(char *cmdline)
{
...
static inline void markKernel_user_CMD(int flag , char * cmdline)
{
	struct tag *kernel_tag = (struct tag*)CMDLINE_TAG_ADDR;
...
UINT32 _ShadowOS(UINT32 nVol, UINT32 PartitionID)
{
...
static void _SelectBootMode(UINT32  PartitionID)
{
...
void exec_memory(unsigned int pJumpAddr ) {
	unsigned int run_address = 0;
	
	__asm__ __volatile__("": : :"memory"); 
	run_address = pJumpAddr;
	
	if (pJumpAddr != NULL)
	{
		_RunImage = (void(*)(int, int))run_address;
...
#else
	PRINTF("\r\n%d 4 P SamyGO 2nd "SAMSUNG_BOOTLOADER_VERSION" (" __DATE__ "-" __TIME__ ")\r\n", flag?2:1);
#endif
#endif
}
int getPartFlag(void)
{
...
static void ResetUsbHub()
{
	...
	*(volatile unsigned int*) 0x30090d00  |= (0x03);
	(*(volatile unsigned int*)0x30090d04) |= (0x01);
	// enable USB controler!!!! for usb rootfs booting
	*(volatile unsigned int*) 0x30090d00  |= (0x30);
	(*(volatile unsigned int*)0x30090d04) |= (0x02);
}
...
void boot_kernel(void) {
	...
	FastLogo();
	s = getenv ("bootargs");
	PRINTF("[ECHO_P] Booting Kernel\n");
	printf("%s\n", (char*)s );
	
	markKernel( (char*)s );
	s = getenv ("bootparition");
	part_flag = s ? (int)simple_strtol(s, NULL, 10) : 0;
	if(part_flag == 0)
	{
...
//	serial_init();
   ...
   ResetUsbHub();
	//PRINTF("Select Boot Parition : (1 or 2) \n");
//#if !defined(SILENT_KERNEL) 
	...
	printf("bootdelay = %d\n",bootdelay );
	s = getenv ("loadaddr");
	ret = (int)simple_strtol(s, NULL, 10);
	set_load_adress(ret);
	printf("Kernel Load Address %08x\n", get_load_adress() );
	s = 0;
	s = getenv ("bootargs");
	if ( s == 0) {
		default_env();
	...
	while(1)
	{
		...
		if (rc != 0) {
			if (rc == 65 || rc == 97 ) { // a
				...
				if(err != SDP_MMC_SUCCESS){
					PRINTF("[ECHO_P] Error MMC Read\n");
				} else {
					FastLogo();
					PRINTF("[ECHO_P] Booting Kernel\n");
					PRINTF("%s", (char*)OS_ONBOOT_ADDR2 );
					markKernel( (char*)OS_ONBOOT_ADDR2 );
					if(part_flag == 0)
					{
						_SelectBootMode(PARTITION_ID_KERNEL0);
					}
