...
#echo "$NM"
	...
	if [ -e $CMAC_FILE  ]
	then
		./cmac_gen.Firenze onboot.bin 2nd_onboot.img
		cp 2nd_onboot.img  /mnt/disk1/TV_rootfs/mtd_down/2nd_onboot.img
		echo "copied 2nd_onboot.img to /mnt/disk1/TV_rootfs/mtd_down/2nd_onboot.img" 
		sync	
	fi
	if [ $dev = y ]; then
