...
#undef USB_DEBUG
#undef USB_DEBUG_DESC	/* ij.jang : dump descriptors for debugging */
#define USB_DEBUG
#ifdef	USB_DEBUG
			...
			if((ep->bmAttributes & USB_ENDPOINT_XFERTYPE_MASK)==USB_ENDPOINT_XFER_CONTROL) {	/* Control => bidirectional */
				...
				if ((ep->bEndpointAddress & 0x80)==0) { /* OUT Endpoint */
					if(ep->wMaxPacketSize > dev->epmaxpacketout[b]) {
						dev->epmaxpacketout[b] = ep->wMaxPacketSize;
						/* ij.jang : debugging */
						USB_PRINTF ("ep->wMaxPacketSize = %04X --> ", ep->wMaxPacketSize);
	...
	while(index + 1 < dev->config.wTotalLength) {
		switch(head->bDescriptorType) {
			...
			case USB_DT_ENDPOINT:
				...
				/* ij.jang : dump for debugging */
#if defined(USB_DEBUG_DESC)
				{
					struct usb_endpoint_descriptor *epd =
						&dev->config.if_desc[ifno].ep_desc[epno];
					USB_PRINTF ("EP DESC: %02X %02X %02X %02X %04X %02X\n", 
							epd->bLength,
							epd->bDescriptorType,
							...
							epd->bRefresh,
							epd->bSynchAddress);
					USB_PRINTF ("Source: %02X %02X %02X %02X %02X %02X %02X\n",
							buffer[index+0],
							buffer[index+1],
							buffer[index+2],
							buffer[index+3],
...
#if defined(USB_DEBUG_DESC)
	{
		printf ("(sizeof) conf=%d, intf=%d, ep=%d\n",
				sizeof(struct usb_config_descriptor),
				sizeof(struct usb_interface_descriptor),
				sizeof(struct usb_endpoint_descriptor));
		/* assume 1 configuration, 1 interface */
		int i;
		struct usb_interface_descriptor *itf = 
			&dev->config.if_desc[0];
		for (i=0; i<itf->no_of_ep; i++) {
			struct usb_endpoint_descriptor *ep =
				&itf->ep_desc[i];
			USB_PRINTF ("EP #%d: %08X %02X %02X %02X %02X %04X %02X\n",
					ep->bEndpointAddress & 0xF, (ulong)ep,
