...
# 2009.06.03 Lee,Hakbong migration to MV Pro 5.0 TridentSX mips24ke_nfp_be
#
...
RPM_PACKAGE_RELEASE="19.0.46.custom"
...
then
  echo ""
  echo "--------------------------------------- "
  echo "GLIBC AutoBuilder 1.0 (SELP) 2009.06.04 "
  echo "--------------------------------------- "
...
CC="mips24ke_nfp_be-gcc " ; export CC ; 
AS="mips24ke_nfp_be-as" ; export AS ; 
LD="mips24ke_nfp_be-ld" ; export LD ; 
AR="mips24ke_nfp_be-ar" ; export AR ; 
RANLIB="mips24ke_nfp_be-ranlib" ; export RANLIB ; 
CPP="mips24ke_nfp_be-cpp " ; export CPP ; 
CXX="mips24ke_nfp_be-g++ " ; export CXX ; 
NM="mips24ke_nfp_be-nm" ; export NM ; 
...
cd objdir && ../configure \
	--host=mips-montavista-linux-gnu \
...
pushd ../LOCALOBJDIR
cp -a ../timezone/* .
tar xvfz $RPM_BUILD_DIR/tzcode2009e.tar.gz
tar xvfz $RPM_BUILD_DIR/tzdata2009g.tar.gz
if [ $? != 0 ]; then echo "SELP: glibc Error Occurred while decompressing tzdata2009g.tar.gz "; exit 1 ;fi
...
(cd $INSTALL_DIR/target/usr/lib/optinfo/libc; umask 022; mips24ke_nfp_be-ar x $OBJECTDIR/libc_pic.a)
mv $INSTALL_DIR/target/usr/lib/optinfo/libc/version.os $INSTALL_DIR/target/usr/lib/optinfo/libc/version.o
(cd $INSTALL_DIR/target/usr/lib/optinfo/libc; umask 022; mips24ke_nfp_be-libindex $OBJECTDIR/libc.so -r *.o -o *.os > index)
...
(cd $INSTALL_DIR/target/usr/lib/optinfo/libm; umask 022; mips24ke_nfp_be-ar x $OBJECTDIR/math/libm_pic.a)
(cd $INSTALL_DIR/target/usr/lib/optinfo/libm; umask 022; mips24ke_nfp_be-libindex $OBJECTDIR/math/libm.so -r *.o -o *.os > index)
...
\$glibc_source/configure --host=mips-montavista-linux-gnu --enable-add-ons --prefix=/usr
...
do
        ...
        mips24ke_nfp_be-objcopy --only-keep-debug "$f" "${debugfn}" || :
        mips24ke_nfp_be-strip --strip-debug "$f" || :
        mips24ke_nfp_be-objcopy --add-gnu-debuglink="${debugfn}" "$f" || :
